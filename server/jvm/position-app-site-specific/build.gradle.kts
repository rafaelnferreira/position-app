    ...
    main {
        contents {
            ...
            into("site-specific/cfg") {
                from("${project.rootProject.projectDir}/position-app-site-specific/src/main/resources/cfg")
                filter(
            ...
            into("site-specific/scripts") {
                from("${project.rootProject.projectDir}/position-app-site-specific/src/main/resources/scripts")
                filter(
                    org.apache.tools.ant.filters.FixCrLfFilter::class,
                    "eol" to org.apache.tools.ant.filters.FixCrLfFilter.CrLf.newInstance("lf")
                )
            ...
            into("site-specific/data") {
                from("${project.rootProject.projectDir}/position-app-site-specific/src/main/resources/data")
                filter(
                    org.apache.tools.ant.filters.FixCrLfFilter::class,
                    "eol" to org.apache.tools.ant.filters.FixCrLfFilter.CrLf.newInstance("lf")
                )
...
tasks {
    distZip {
        archiveBaseName.set("position-app-site-specific")
...
publishing {
    publications {
        create<MavenPublication>("position-appSiteSpecificDistribution") {
...
artifactory {
    ...
    val publicationNames = arrayOf("position-appSiteSpecificDistribution")
    publish(
